...
def filtermfunc(filterraw):
    ...
    caster = []
    casterstr = "spell.id IN (SELECT sid FROM spellcaster WHERE cid == {})"
    ...
    for term in filterraw: #iterates through the returned terms from the form in the "search" page
        ...
        if term[0] == "school": 
            ...
            term [1] = entfields[2].index(term[1])
            caster.append(casterstr.format(str(term[1])))
